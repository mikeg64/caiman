...
%%
    ...
    nameAndDate                             = strcat(newImageName{kk,1},dateString);
    %disp(nameAndDate)
    %disp(oldImagesList)
    %--------------------------------------------------------------------------------------------------
    ...
    if ~any(cell2mat(strfind(oldImagesList,nameAndDate)))
        ...
        try
            ...
            try
                ...
                switch newImageName{kk,1}(end-1:end)
                    ...
                    case 'MI'
                        [data_stats,dataOut]   = cellMigration(dataIn(1:2:end,1:2:end,:));
                        ...
                        outputMessage{9}    = ' ';
                        outputMessage{10}   = ' ';
                        %outputMessage=strvcat('',outputMessage1,'',outputMessage2,outputMessage3,outputMessage4,outputMessage5,outputMessage6,' '); %#ok<VCAT>
                    case 'PE'
        ...
        %--------------------------------------------------------------------------------------------------
        % 6 save results (name) to the file of the processed images (only if successfull)
        %--------------------------------------------------------------------------------------------------
