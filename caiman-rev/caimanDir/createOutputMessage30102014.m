...
switch caseRoutine
    ...
    case 'SE'
        if exist('extraData','var')
            ...
            outputMessage{11}    = strcat('Number of objects with NumEuler<1 = ',      num2str(sum(  [data_stats.statsObjects.EulerNumber]'<1)),' ');
            outputMessage{7}    = strcat('Average Eccentricity                 = ',      num2str(mean( [data_stats.statsObjects.Eccentricity]')),' ');
            outputMessage{9}    = strcat('Average Wall thickness             = ',      num2str(mean( [data_stats.avThickness] )),' ');
            outputMessage{8}    = strcat('Average lumen/Vessel Area    = ',      num2str(mean( [data_stats.luVA] )),' ');           
            outputMessage{10}    =strcat('Average roundness                = ',      num2str(mean( [data_stats.roundness] )),' ');;
            outputMessage{11}    = ' ';
            outputMessage{12}    = ' ';
